...
describe('WeatherController', () => {
  ...
  beforeEach(() => {
    mockWeatherService = new WeatherService({ apiKey: 'test', baseUrl: 'test' }) as jest.Mocked<WeatherService>;
  ...
  describe('GET /weather', () => {
    it('should return weather by city', async () => {
      ...
      mockWeatherService.getWeatherByCity = jest.fn().mockResolvedValue(mockWeather);
    ...
    it('should return weather by coordinates', async () => {
      ...
      mockWeatherService.getWeatherByCoordinates = jest.fn().mockResolvedValue(mockWeather);
      ...
      expect(response.status).toBe(200);
      expect(response.body).toEqual(mockWeather);
      expect(mockWeatherService.getWeatherByCoordinates).toHaveBeenCalledWith(48.8566, 2.3522);
    });
    ...
    it('should return 400 when no parameters are provided', async () => {
      ...
      expect(response.body.error).toBe('Missing required parameters: either city or lat/lng must be provided');
    });
    ...
    it('should handle errors from weather service', async () => {
      mockWeatherService.getWeatherByCity = jest.fn().mockRejectedValue(new Error('City not found'));
